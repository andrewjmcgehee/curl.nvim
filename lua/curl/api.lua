...
M.pick_global_collection = function()
  ...
  vim.ui.select(global_collections, { prompt = "Open Global Collection:" }, function(selection)
...
M.pick_scoped_collection = function()
  ...
  vim.ui.select(scoped_collections, { prompt = "Open Scoped Collection:" }, function(selection)
    if selection == nil then
      return
    end
...
M.delete_global_collection = function()
  local global_collections = cache.get_collections(true)
  vim.ui.select(global_collections, { prompt = "Delete Global Collection:" }, function(selection)
    if selection == nil then
      return
    end
    local filename = cache.load_custom_command_file(selection, true)
    assert(os.remove(filename))
  end)
end

M.delete_scoped_collection = function()
  local scoped_collections = cache.get_collections(false)
  vim.ui.select(scoped_collections, { prompt = "Delete Scoped Collection:" }, function(selection)
    if selection == nil then
      return
    end
    local filename = cache.load_custom_command_file(selection)
    assert(os.remove(filename))
  end)
end
